...
def accumulate(merger, start, n, term):
    ...
    products = start
    while i <= n:
        products, i = merger(products, term(i)), i+1
    return products
...
def mul_by_num(num):
    ...
    -8
    """
    return lambda x: mul(num,x)   #最典型的lambda表达式
...
def mod_maker():
    ...
    True
    """
    return lambda x, y: x % y or not False    #False 即 x%y==0，not False 即True，当x%y==0时，即为假值，or语句优先返回真值
